...
func buildRewardedAdHtml(
    ...
    let jsonData  = (try? JSONSerialization.data(withJSONObject: config)) ?? Data()
    let jsonStr   = String(data: jsonData, encoding: .utf8) ?? "{}"
    // Prevent </script> from closing the tag prematurely
    let safeJson  = jsonStr.replacingOccurrences(of: "</", with: "<\\/")

    let bridge = "window.__paBridge=function(e,d){window.webkit.messageHandlers.peerads.postMessage({event:e,data:d});};"
    let inject = "window.__PA=\(safeJson);\(bridge)"
    ...
    <body><script>\(inject)</script>\(PA_BODY)<script>\(PA_JS)</script></body></html>
    """
}
